    ...
    return (
      <div className="ml-2">
        ...
        >
          ...
          <span className="font-medium">{node.prefix.replace(/\/$/, "").split("/").pop()}</span>
        </div>
  ...
  if (node.type === "file") {
    return (
      ...
      >
        ...
        <span className="ml-auto text-xs text-gray-400">{(node.size / 1024).toFixed(1)} KB</span>
      </div>
    );
  }
    ...
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <Card className="p-6 shadow-lg rounded-xl border border-gray-100">
        ...
        <div className="space-y-2 max-h-[500px] overflow-y-auto">
          {data.length > 0 ? (
            data.map((node, i) => <TreeNode key={i} node={node} onFileClick={handleFileClick} />)
          ) : (
        ...
        <Dialog open={!!selectedFile} onOpenChange={closePreview}>
          <DialogContent className="max-w-3xl p-6 rounded-xl">
            <DialogHeader className="flex justify-between items-center mb-4">
              <DialogTitle className="text-lg font-semibold">{selectedFile.key.split("/").pop()}</DialogTitle>
            ...
            <div className="mt-4">
              {selectedFile && signedUrl ? (
                selectedFile.key.match(/\.(jpg|jpeg|png|gif)$/i) ? (
                  <img src={signedUrl} alt={selectedFile.key} className="max-h-[500px] mx-auto rounded-lg shadow-lg" />
                ) : selectedFile.key.match(/\.(txt|md)$/i) ? (
                  <iframe src={signedUrl} className="w-full h-[500px] rounded-lg border" />
                ) : (
                  <p className="text-gray-600 text-center">No preview available for this file type.</p>
                )
              ) : (
